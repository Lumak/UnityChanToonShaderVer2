...
////// Lighting:
                ...
                float node_7239 = (saturate((_Set_HighColorMask_var.g+_Tweak_HighColorMaskLevel))*lerp( (1.0 - step(node_6430,(1.0 - _HighColor_Power))), pow(node_6430,exp2(lerp(11,1,_HighColor_Power))), _Is_SpecularToHighColor ));
                float3 node_2397 = (lerp( _HighColor.rgb, (_HighColor.rgb*Set_LightColor), _Is_LightColor_HighColor )*node_7239);
                float node_7081 = Set_FinalShadowMask;
                float3 Set_HighColor = (lerp( saturate((node_3441-node_7239)), node_3441, _Is_BlendAddToHiColor )+lerp( node_2397, (node_2397*((1.0 - node_7081)+(node_7081*_TweakHighColorOnShadow))), _Is_UseTweakHighColorOnShadow ));
                float3 node_3159 = Set_HighColor;
                float2 node_3630 = Set_UV0;
                float4 _Set_RimLightMask_var = tex2D(_Set_RimLightMask,TRANSFORM_TEX(node_3630, _Set_RimLightMask));
                ...
                float node_9686 = 1.0;
                float node_6470 = 0.0;
                float node_8547 = saturate(lerp( (node_6470 + ( (node_3992 - _RimLight_InsideMask) * (node_9686 - node_6470) ) / (node_9686 - _RimLight_InsideMask)), step(_RimLight_InsideMask,node_3992), _RimLight_FeatherOff ));
                float node_6920 = 0.5*dot(i.normalDir,lightDirection)+0.5;
                float3 _LightDirection_MaskOn_var = lerp( (_Is_LightColor_RimLight_var*node_8547), (_Is_LightColor_RimLight_var*saturate((node_8547-((1.0 - node_6920)+_Tweak_LightDirection_MaskLevel)))), _LightDirection_MaskOn );
                float node_6788 = pow(node_4927,exp2(lerp(3,0,_Ap_RimLight_Power)));
                float3 Set_RimLight = (saturate((_Set_RimLightMask_var.g+_Tweak_RimLightMaskLevel))*lerp( _LightDirection_MaskOn_var, (_LightDirection_MaskOn_var+(lerp( _Ap_RimLightColor.rgb, (_Ap_RimLightColor.rgb*Set_LightColor), _Is_LightColor_Ap_RimLight )*saturate((lerp( (node_6470 + ( (node_6788 - _RimLight_InsideMask) * (node_9686 - node_6470) ) / (node_9686 - _RimLight_InsideMask)), step(_RimLight_InsideMask,node_6788), _Ap_RimLight_FeatherOff )-(saturate(node_6920)+_Tweak_LightDirection_MaskLevel))))), _Add_Antipodean_RimLight ));
                ...
                float3 _Is_LightColor_AR_var = lerp( node_6611, (node_6611*Set_LightColor), _Is_LightColor_AR );
